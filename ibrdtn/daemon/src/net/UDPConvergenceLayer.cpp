	...
	namespace net
	{
		...
		void UDPConvergenceLayer::onUpdateBeacon(const ibrcommon::vinterface &iface, DiscoveryBeacon &announcement) noexcept(false)
		{
			...
			try
			{
				...
				if (!crosslayer)
					throw ibrcommon::Exception("crosslayer discovery disabled!");
				...
				if (list.empty())
					throw ibrcommon::Exception("no address found");
				...
				for (std::list<ibrcommon::vaddress>::const_iterator addr_it = list.begin(); addr_it != list.end(); ++addr_it)
				{
					...
					if (addr.scope() != ibrcommon::vaddress::SCOPE_GLOBAL)
						continue;
		...
		void UDPConvergenceLayer::queue(const dtn::core::Node &node, const dtn::net::BundleTransfer &job)
		{
			...
			try
			{
				...
				if (size > m_maxmsgsize)
				{
					...
					if (!dtn::daemon::Configuration::getInstance().getNetwork().doFragmentation() && !bundle.get(dtn::data::PrimaryBlock::DONT_FRAGMENT))
					{
